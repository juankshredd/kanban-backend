...
4. **`render.yaml`** — Blueprint defining the web service (`plan: free`, `runtime: node`, `autoDeployTrigger: off` since GitHub Actions triggers deploys via the hook instead of Render's native push trigger, `buildCommand: npm ci && npm run migrate`, `startCommand: npm start`, `healthCheckPath: /test-db`) and a `free`-plan Postgres database, with the web service's DB env vars wired via `fromDatabase`. Migrations run in `buildCommand` rather than `startCommand`/`preDeployCommand`: the free plan doesn't support `preDeployCommand`, and anything in `startCommand` re-runs on every wake-from-sleep (free services idle out after ~15 min), not just on real deploys.
